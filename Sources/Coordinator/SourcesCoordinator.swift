...
struct SourceElement<Element> where Element: DataSource {
    ...
    let context: ListCoordinatorContext<Element.Model>
    let offset: Int
    let count: Int

    var coordinator: ListCoordinator<Element.Model> { context.listCoordinator }
...
class SourcesCoordinator<C: RangeReplaceableCollection, Model>: ListCoordinator<Model>
where
    C.Element: DataSource,
    C.Element.Model == Model
{
    enum SubsourceType {
...
//        }
    ...
    typealias Subcoordinator = ListCoordinator<C.Element.Model>
    typealias Related = ListCoordinatorContext<C.Element.Model>
    typealias Subsource = SourceElement<C.Element>
    ...
    lazy var subsourcesAndSubsectioned = toSubsources(source, sectioned: nil) //subsourceType.from!(source), sectioned: nil)
    ...
    var subsourcesArray: ContiguousArray<C.Element> {
        var sources = ContiguousArray<C.Element>(capacity: subsources.count)
...
//        update: ListUpdate<SourceBase>.Whole
        ...
        subsourceType = .values
        super.init(options: .removeEmptySection)
    ...
    func toSubsources(
        ...
        func addModelSources() {
            let coordinator = SourcesCoordinator<[C.Element], Model>(
...
//                update: .init(way: update.way)
    ...
    func addContext(to context: ListCoordinatorContext<C.Element.Model>) {
...
//        }
    ...
    func subContextIndex(at indexPath: IndexPath) -> (SourceElement<C.Element>, IndexPath) {
