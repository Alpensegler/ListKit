...
let allSelectors: Set<Selector> = {
    var selectors: Set<Selector> = {
        ...
        if #available(iOS 14, *) {
            sels.insert(#selector(UICollectionViewDataSource.indexTitles(for:)))
            sels.insert(#selector(UICollectionViewDataSource.collectionView(_:indexPathForIndexTitle:at:)))
        }
        return sels
    }()
