...
public protocol Index: Hashable {
    var section: Int { get set }
    func contextIndex(at indices: ContiguousArray<(indices: [Int], index: Int?)>) -> Int?
    func offseted(section: Int, item: Int) -> Self
}
...
extension IndexPath: ListIndex {
    ...
    public static var isSection: Bool { false }

    public func contextIndex(at indices: ContiguousArray<(indices: [Int], index: Int?)>) -> Int? {
        let indices = indices[section]
        if indices.indices.count > item { return indices.indices[item] }
        return indices.index
    }

    public func offseted(section: Int, item: Int) -> IndexPath {
        [self[0] + section, self[1] + item]
    }
//
//    var section: Int {
//        get { self[startIndex] }
...
//    }
    ...
    public func offseted(_ index: IndexPath, plus: Bool = true) -> IndexPath {
...
extension Int: ListIndex {
    public static var isSection: Bool { true }
    public var section: Int { get { self } set { self = newValue } }

    public func contextIndex(at indices: ContiguousArray<(indices: [Int], index: Int?)>) -> Int? {
        indices[self].index
    }

    public func offseted(section: Int, item: Int) -> Int {
        self + section
    }
//    var item: Int { 0 }
//
...
//    func offseted(_ offset: Int, isSection: Bool) -> Int { isSection ? self + offset : self }
    public func offseted(_ index: Int, plus: Bool) -> Int { self + (plus ? index : -index) }
